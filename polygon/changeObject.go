...
func CreateObjectForScene(objects []inter.Square, a, b, a2, b2 int, obj, do string, step float64) []inter.Square {
	for i := 0; i < len(objects); i++ {
		if objects[i].NumberX == a && objects[i].NumberY == b {
			if do == "создать" {
				objects[i].Object = len(objects)
				if a2 == 0 && b2 ==0 {
					objects = createObj(objects, obj, step, objects[i].Triagle1.P1.X, objects[i].Triagle1.P1.Z, 0, 0, i)
				} else {
					for j := 0; j < len(objects); j++ {
						if objects[j].NumberX == a2 && objects[j].NumberY == b2 {
							fmt.Println("+")
							objects = createObj(objects, obj, step, objects[i].Triagle1.P1.X, objects[i].Triagle1.P1.Z, objects[j].Triagle1.P1.X, objects[j].Triagle1.P1.Z, i)
							break
						}
					}
				}
...
func createObj(square []inter.Square, obj string, step float64, x, z float64, x2, z2 float64, k int) []inter.Square {
	switch obj {
		case "станция":
		...
		case "головной вагон":
			...
			square = append(square, createTrainHead(x, z, x2, z2, step)...)
		case "вагон":
			fmt.Println("+")
...
func createTrainHead(x, z, x2, z2, step float64) []inter.Square {
	slice := make([]inter.Square, 11)
	k := 0

	// fmt.Println(x2, z2, x, z)
	...
	slice[k].Triagle1 = createHead(x2 + step / 5, 0.255, z2, x2 + step / 5, 0.53, z2 + step, color.NRGBA{245, 243, 240, 255})
	slice[k].Triagle2 = createHead(x2 + step / 5 * 4, 0.255, z2, x2 + step / 5 * 4, 0.53, z2 + step, color.NRGBA{245, 243, 240, 255})
	k++

	slice[k] = createSquareLeftRight(x2 + step / 5, 0.205, z2, x2 + step / 3 * 2, 0.255, z2 + step, color.NRGBA{245, 243, 240, 255})
	k++
	slice[k] = createSquareLeftRight(x2 + step / 5 * 4, 0.205, z2, x2 + step / 5 * 4, 0.255, z2 + step, color.NRGBA{245, 243, 240, 255})
	k++

	slice[k] = createSquareBackFront(x2 + step / 5, 0.205, z2 + step, x2 + step / 5 * 4, 0.255, z2 + step, color.NRGBA{245, 243, 240, 255})
	k++

	// slice[k] = createSquareUpDown(x2 + step / 5, 0.255, z2, x2 + step / 5 * 4, 0.53, z2 + step, color.NRGBA{245, 243, 240, 255})	
	// k++

	//slice[k].Triagle1 = createTriangleBTA(x1, y1, z1, x2, y2, z2, clr)
	//slice[k].Triagle2 = createTriangleBTB(x1, y1, z1, x2 , y2, z2, clr)
